...
- "Birthplace" to geospatial data. To do so, click on the Abc symbol and select "Geographic Role" -> "Country." The symbol for the column should have changed from Abc to a Globe now.
...
6. Update the x-axis label to something clearer by clicking on the current label, "Number of Records." In the pop-up that appears, type "Number of Books" for the new label. Click the "X" when finished. 
...
4. To give our map access to additional information that we will use later when making our dashboard, drag the "Title" and "Book Fullurl" pills over the "Tooltip" mark.
