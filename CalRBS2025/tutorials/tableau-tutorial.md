...
but Tableau also recently launched free Tableau Desktop for everyone. (Probably because they're now promoting [Tableau Next](https://www.tableau.com/products/tableau-next), their AI-assisted platform.)
...
### Save Your Work
To save your Tableau workbook, you have to create a free Tableau account. When you click save, you will be prompted to log in and save your workbook to Tableau's server, where it will be visible on Tableau Public. 
Users can interact with your workbook on Tableau Public, or they could download, add to, or otherwise edit it. Remember, your data also becomes public, unless you're working with Tableau Dekstop! 

Recently, Tableau Public added a new "Save" button that saves your work locally, without promoting to Tableau Public. We'll use that button for the rest of our saves.  
...
- For the URL, click on the triangle  and select "ATTR(Book Fullurl)." (Ignore the warning about the URL.)
...
#### Challenge: How to handle multiple titles published in the same year and country?  
For example, look at Germany and the United Kingdom in 1978.  

Approach 1:  
1. Go back to the map worksheet and add "Book Title" to Details. Add all members to show the first book alphabetically. But what if you want to interactively choose which books are displayed?

Approach 2:  
1. Create a new worksheet -> double click "Book Title" and drag "Book Fullurl" to "Tooltip". Call it "book titles".
2. Create a new dashboard -> add the map, the book titles worksheet, and a web page placeholder.
3. Add the URL Action as before (see step 4 above for a refresher), but this time, ensure both source sheets are selected.
4. Click on the map and select the "Use As Filter" button (it looks like a funnel cup).
Now when you click a point on the map, the book titles list adjusts to show the books for that point, allowing you to pick which page loads in Goodreads.

### Story: Contextualize your Visualizations
Let's pull our visualizations together into a story.
1. Start a story by clicking on the icon that looks like an open book (it's next to the new dashboard icon).
...
* Experiment with Tableau's new "Viz Extensions" (find under "Show Me") to extend what you can create in the platform.  
